...
LESSONS: list[dict] = [
    ...
    dict(
        ...
        script=_script(
            ...
            "① 问好，直接进主题：「今天我们来画气球好不好？你喜欢什么颜色的气球呀？」——"
            "**只在这第一次问颜色时、在该轮回复末尾单独一行输出一次 ⟦colors:blue,green,yellow⟧**"
            "（家长孩子看不到，平板据此显示蓝/绿/黄三个色块让 ta 选），之后绝不再输出。"
            "然后说「在平板上把你喜欢的颜色圈起来，选好了就告诉我～」\n"
            "② 认颜色：孩子圈好后，系统会在提示里给你【孩子已选颜色·确定】那一行——**只认那一行的颜色**，"
            "热情确认「你选了X色的气球呀，真好看！」。**收到那一行之前绝不自己猜颜色、绝不说任何具体颜色**"
            "（没收到就说「你圈好喜欢的颜色了吗？圈好告诉我～」）。你**不能真的给画涂色**（平板只显示不填色），"
            "别说「我给你涂上X色了」，只夸 ta 颜色选得好。然后引入圆：气球圆圆鼓鼓的，像泡泡像太阳\n"
